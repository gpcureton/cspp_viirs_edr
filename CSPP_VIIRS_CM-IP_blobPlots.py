...
class VCMclass():
    ...
    def __init__(self,xmlDir,blobPath,endian=adl_blob.LITTLE_ENDIAN):
        ...
        self.dataName = {}
        self.dataName['VIIRS-CM-IP'] = 'vcm0'

        self.blob_dict = get_blob_asc_dict(xmlDir,blobPath,self.collShortNames)

        for granID in  self.blob_dict['VIIRS-CM-IP'].keys() :
            xmlFile = path.join(xmlDir,self.xmlName['VIIRS-CM-IP'])
            blobFile = self.blob_dict['VIIRS-CM-IP'][granID][0]
            blobFile = path.join(blobPath,'%s'%(blobFile))
            blobObj = adl_blob.map(xmlFile,blobFile,endian=endian)
            self.blob_dict['VIIRS-CM-IP'][granID].append(blobObj)
    ...
    def plot_VCM_granules(self,pngDir=None,annotation='',dpi=300):
        ...
        for shortName in collShortNames :
            ...
            for granID in  blobDict[shortName].keys() :
                ...
                im = ax.imshow(ma.masked_less(data,0),interpolation='nearest',vmin=vmin,vmax=vmax,cmap=cmap)
