...
# encoding: utf-8
"""
MMAB-SeaIceConc_toBlob.py

The purpose of this script is to ingest MMAB grib files and convert
to MMAB blob format required by Raytheon's Algorithm Development Library (ADL).

Minimum commandline(s)...

python MMAB-SeaIceConc_toBlob.py -g gribFile              # To audit a grib file...
python MMAB-SeaIceConc_toBlob.py -x xmlFile -g gribFile   # To transcode a grib file...

Created by Geoff Cureton on 2013-03-22.
...
import logging
LOG = logging.getLogger('MMAB-SeaIceConc_toBlob')


class MMABclass(object):
    ...
    MMAB_LAYER_VALUES = np.array([10.0, 20.0, 30.0, 50.0, 70.0, 100.0, 150.0, 200.0, 
    ...
    def __init__(self,gribFile=None,isListing=False):
        if gribFile is not None :
            ...
            self.__MMABaudit(gribFile,returnObj=returnObj,levelInhPa=None)

        else :
            pass

    class getMMABlevelMessage(object):
        '''
        getMMABlevelMessage

        Instantiations of the getMMABlevelMessage class serve as containers for multiple 
        getMMABmessage objects making up a profile, and have as attributes various properties 
        common to each pressure level. The getMMABmessage object for each level contains the array 
        ...
        def __init__(self,fileObj,name,parameterName,shortName,units,parameterUnits,typeOfLevel,level,pressureUnits):
            ...
            for message in fileObj.msgList :
                ...
                self.messageLevelData[pressureLevel] = MMABclass.getMMABmessage(self,name,parameterName,shortName,units,parameterUnits,typeOfLevel,pressureLevel,pressureUnits)
                self.messageLevelData[pressureLevel].height = "%s %s" %(pressureLevel,self.pressureUnits)

    class getMMABmessage(object):
        '''
        getMMABmessage

        Instantiations of the getMMABmessage class contain the array data for a single GRIB message,
    ...
    @staticmethod
    def MMABfileAudit(gribFile,levelInhPa=None):
        '''
        MMABfileAudit
        ...
        '''
        gribFile = path.abspath(path.expanduser(gribFile))
        MMABobj = MMABclass(gribFile=gribFile, isListing=True)

    def __MMABaudit(self,gribFile,returnObj=False,levelInhPa=None) :
        '''
        __MMABaudit

        This private method opens the supplied grib file, and populates a dictionary with getMMABlevelMessage 
        and getMMABmessage objects matching the required ADL blob datasets.
        ...
        gfsMsgKeys = MMABclass.gfsMsgKeys
        ...
        for msgKey in gfsMsgKeys :
            ...
            name = MMABclass.ancName[msgKey]
            shortName = MMABclass.ancShortName[msgKey]
            typeOfLevel = MMABclass.ancTypeOfLevel[msgKey]

            if (typeOfLevel == 'isobaricInhPa') :
                level = MMABclass.ancLevel[msgKey] if (levelInhPa==None) else levelInhPa
            else :
                level = MMABclass.ancLevel[msgKey]
            ...
            if (numMessages>0) :
                ...
                if not returnObj :
                    ...
                    if (msgList[msgListIdx]['typeOfLevel'] == 'isobaricInhPa') :
                        ...
                        print "level = ",level
                        self.MMABmessages[msgKey] = self.getMMABlevelMessage(self,name,parameterName,shortName,units,parameterUnits,typeOfLevel,level,pressureUnits)
                        
                    else :

                        self.MMABmessages[msgKey] = self.getMMABmessage(self,name,parameterName,shortName,units,parameterUnits,typeOfLevel,level,pressureUnits)
                        ...
                        self.MMABmessages[msgKey].height = height
    ...
    def MMABgribToBlob_interp(gribObj,xmlFile,newMMABblob,endian=adl.LITTLE_ENDIAN):
        '''
        MMABgribToBlob_interp

        Method to copy the contents of the grib object gribObj to a newly
        created ADL blob file newMMABblob. If the grib lat/lon grid is coarser 
        ...
        newBlobObj = adl.create(xmlFile,newMMABblob,endian=endian,overwrite=True)

        for msgKey in gribObj.gfsMsgKeys :
            print "\nCopying MMAB[%s] to blob...\n" % (msgKey)
            msgObj = gribObj.MMABmessages[msgKey]
            blobArr = newBlobObj.__getattribute__(msgKey)

            try :
                if needsInterp :
                    z = gribObj.MMABmessages[msgKey].data
                    ...
                    for row in np.arange(gribObj.Nlats):
                        blobArr[row][:] = gribObj.MMABmessages[msgKey].data[row,:]
    ...
    def MMABgribToBlob_interpNew(gribObj,xmlFile,newMMABblob,endian=adl.LITTLE_ENDIAN,reverseLat=False):
        '''
        MMABgribToBlob_interpNew
        ...
        for msgKey in gribObj.gfsMsgKeys :
            ...
            try :
                if needsInterp :
                    ...
                    blobArr[:,:] = gribObj.MMABmessages[msgKey].data[::latDir,:]
...
def main():
    ...
    description = '''Transcode an MMAB GFS or GDAS grib file into the binary blob format required by Raytheon's Algorithm Development Library (ADL).'''
    ...
    mandatoryGroup.add_option('-g','--grib_file',
                      ...
                      dest="gribFile",
                      type="string",
                      help="The full path of the MMAB grib file")
    ...
    optionalGroup.add_option('-l','--list',
                      ...
                      dest="isListing",
                      #default=False,
                      help="List the datasets contained in this grib file which match the required datasets for the ADL MMAB blob file, and exit.")
    ...
    if options.isListing :
        ...
        MMABclass.MMABfileAudit(options.gribFile,levelInhPa=None)
        ...
        if options.outputFile is None :
            options.outputFile = "MMAB-ANC-Int_grib"

        # Create the grib object and populate with the grib file data
        gribObj = MMABclass(gribFile = options.gribFile)
        ...
        #MMABclass.MMABgribToBlob_interp(gribObj,options.xmlFile,options.outputFile,endian=endian)
        MMABclass.MMABgribToBlob_interpNew(gribObj,options.xmlFile,options.outputFile,\
